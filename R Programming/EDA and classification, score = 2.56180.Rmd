...
invisible(lapply(names(train),function(.name) set(train, which(is.infinite(train[[.name]])), j = .name,value =0)))
      train[is.na(train)] <- 0

model <- LiblineaR(train, target, type = 7, verbose = TRUE)
rm(train)
gc()
...
#Build submission.
submit <- data.frame(predict(logisticL2_Dual,test, proba = TRUE)$probabilities[, levels(target)])
