...
date = strptime(train2$date,"%Y-%m-%d %H:%M:%S")
...
dfTmpMax <- sqldf('select category_predict,pd_district,CatHour,Month,DayDec, count(*) nro from train2 group by category_predict,pd_district,CatHour,Year,Month,Day order by 2')
dfTrainFinal <- ddply(dfTmpMax,~CatHour+pd_district+Month+DayDec,function(x){x[which.max(x$nro),]})
...
split=0.80
trainIndex <- createDataPartition(dfTrainFinal$category_predict, p=split, list=FALSE)
data_train2 <- dfTrainFinal[ trainIndex,]
data_test2 <- dfTrainFinal[-trainIndex,]

print("Debug 120")
model <-  naiveBayes(category_predict ~ ., data = data_train2)
naiveBay <- train(x = data_train2[,c(2:5)], y = data_train2$category_predict, 'nb')
